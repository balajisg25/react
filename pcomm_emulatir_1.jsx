...
function App() {
  ...
  const connectToPCOMM = () => {
    ...
    if (connectionParams.wsFile) {
      // Connect using WS file
      addLog(`Connecting using WS file: ${connectionParams.wsFile}`);
      
      // Simulate parsing WS file to extract connection info
      const wsFileName = connectionParams.wsFile.split('\\').pop();
      
      // Simulate WS file connection
      setTimeout(() => {
        setConnectionStatus('Connected');
        updateEmulatorScreen(`IBM PCOMM Terminal Connected\nUsing WS File: ${wsFileName}\n\nSession properties loaded from WS file\n\nReady for input...`);
        addLog('Connected to PCOMM successfully using WS file');
        setShowConnectionDialog(false);
      }, 1500);
    } else if (!connectionParams.host) {
      // Manual connection requires host
      addLog('Error: Host address is required for manual connection');
      return;
    } else {
      // Manual connection with provided parameters
